...
import type { DashboardAccountMetrics } from "../types";
...
function buildRows(accounts: DashboardAccountMetrics[]): GoalRowState[] {
  return accounts.map((account) => ({
    accountId: account.id,
    accountName: account.name,
    accountValue: account.value,
    hasSavedGoal: !!account.goal,
    initialTargetSpend:
...
export function DashboardGoalsDialog({
  ...
  useEffect(() => {
    if (open) {
      setRows(buildRows(accounts));
    }
  }, [accounts, open]);

  const summary = useMemo(() => {
    const goalsCount = rows.filter((row) => row.hasSavedGoal).length;
    return {
      goalsCount,
      missingCount: Math.max(rows.length - goalsCount, 0),
    };
  }, [rows]);
      ...
      <DialogContent className="max-h-[88vh] max-w-6xl overflow-hidden rounded-[28px] border-slate-200 bg-white p-0">
        ...
        <div className="space-y-4 overflow-auto px-6 py-5">
          <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-600">
            ...
            <Badge variant="outline" className="rounded-full px-3 py-1">
              Cadastradas: {formatInteger(summary.goalsCount)}
            </Badge>
            <Badge variant="outline" className="rounded-full px-3 py-1">
              Pendentes: {formatInteger(summary.missingCount)}
            </Badge>
          </div>
