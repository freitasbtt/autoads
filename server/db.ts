...
export async function ensureDashboardSyncSchema(): Promise<void> {
  ...
  try {
    ...
    // Instalações anteriores usavam uma versão de meta_ad_insights_daily com
    // `date`, `actions` e `cost_per_action_type`. CREATE TABLE IF NOT EXISTS
    // não altera essa tabela existente, portanto compatibilizamos o esquema
    // antes de criar os índices e de permitir os upserts do sincronizador.
    await client.query(`
